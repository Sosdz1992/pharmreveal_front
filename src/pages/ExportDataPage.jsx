...
const ExportDataPage = () => {
    ...
    const valueLengths = {
        ...
        importDate: selectedRange !== null ? "Date From: " + selectedRange?.dateFrom + " Date To: " + selectedRange?.dateTo: "Date From: " + dayjs().subtract(1, 'year').format("YYYY-MM-DD") + "Date To: " + dayjs().format("YYYY-MM-DD"),
        priceSource: priceSource?.length || 0,
    }
    const fetchAll = async () => {
        setLoading(true);
        try {
            const [res1, res2, res3, res4, res5, res6, res7, res8, res9, res10, res11, res12, res13, res14, res15, res16, res17,res18
            ] = await Promise.all([
                ...
                axios.get(`/api/user-access/reference/year?userId=${payload.id}`),
            ...
            setAllYear(res18.data); // сохраняем ответ drug-form
            setYear(res18.data.map(opt => opt.name))
        } catch (e) {
    ...
    const toggleColumn = (colOrValue) => {
        const col = typeof colOrValue === "string"
            ? [...columns, ...columns2].find(c => c.value === colOrValue)
            : colOrValue;

        if (!col) return; // защита, если value не найден в списке

        setSelected(prev => {
            const exists = prev.some(item => item.value === col.value);
            return exists
                ? prev.filter(item => item.value !== col.value)
                : [...prev, col];
        });
    ...
    const handleCheckboxToggle = (field) => {
        const triggerFields = ["year", "segment", "tradeName", "manufacturingCompany",
